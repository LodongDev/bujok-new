...
async function farmFromVillage(cdp, sessionId, baseUrl, sourceVillage, options = {}) {
    ...
    let lastAvailable = {};
    let totalRowsSeen = 0;  // 1단계에서 본 row 총수 (mode 통과 후) — stage 2 결정용
    ...
    for (let page = 1; page <= maxPages; page++) {
        ...
        // mode에서 row 후보로 본 개수 누적 (skip된 것도 포함)
        totalRowsSeen += actions.length + minSkipped;
        ...
        if (actions.length === 0) {
            ...
            // 1단계 page 비어있어도 즉시 2단계 가지 않음.
            // 모든 페이지 처리 완료 후 마지막에 한 번만 (병력 남았을 때만) 2단계 진입.
    ...
    // 2단계 진입 조건 — 1단계에서 row가 0개일 때만 (보낼 마을이 아예 없는 상황)
    // 사용자 명시: "늘 언제나 보낼마을 있는지 체크먼저하고 없으면 커랜틀리어택체크하고"
    // 즉, 1단계에 row가 있으면 (병력 부족 스킵 포함) 2단계 가지 않음
    if (!options._filtersExpanded) {
        if (totalRowsSeen === 0) {
            log.info(`[동줍] ${sourceVillage.name} — 1단계 row 0개 → 2단계 진입 (공격중 마을 포함)`);
            ...
            log.info(`[동줍] ${sourceVillage.name} — 1단계 완료 (${totalSent}회 발사, ${totalRowsSeen}개 row 봄), 보낼 마을 있음 → 2단계 스킵`);
        }
    }
