...
async function farmFromVillage(cdp, sessionId, baseUrl, sourceVillage, options = {}) {
    ...
    for (let page = 1; page <= maxPages; page++) {
        ...
        // C 모드 최소 병력 임계 — 사용자 지정
        // 1) 총 합산 ≥ 6 (1마리만 보내는 경우 절대 없어야 함)
        // 2) 기마(light) 사용되는 경우 light ≥ 6
        const C_MIN_TROOPS = options.cMinTroops != null ? options.cMinTroops : 6;
        const C_MIN_LIGHT = options.cMinLight != null ? options.cMinLight : 6;
        const sumReq = (req) => {
            if (!req) return 0;
        ...
        const cTooSmall = (req) => {
            ...
            if (total > 0 && total < C_MIN_TROOPS) return true;
            if ((req.light || 0) > 0 && (req.light || 0) < C_MIN_LIGHT) return true;
            return false;
        };

        // 각 row별로 어떤 방식으로 공격할지 결정
        // C 모드 최소 병력 미달이면 폴백도 안 함 (어떤 방식으로도 공격 안 보냄)
        const actions = [];
        let cMinSkipped = 0;
        for (const row of data.rows) {
            if (mode === 'C' || mode === 'C_OR_A' || mode === 'C_OR_B') {
                if (row.hasC && row.reportId) {
                    if (cTooSmall(row.requiredC)) { cMinSkipped++; continue; }
        ...
        if (cMinSkipped > 0) log.info(`[동줍] ${sourceVillage.name} p${page} — C 최소 병력 미달로 ${cMinSkipped}개 스킵 (총<${C_MIN_TROOPS} 또는 기마<${C_MIN_LIGHT})`);
    ...
    // 1단계 완료 후 자동으로 2단계 진입 — 공격중 마을 포함 추가 발사 (병력 남았으면)
    // 2단계는 30분 쿨다운으로 동일 타겟 spam 방지
    if (!options._filtersExpanded) {
        log.info(`[동줍] ${sourceVillage.name} — 1단계 완료 (${totalSent}회) → 2단계 진입 (공격중 마을 포함)`);
