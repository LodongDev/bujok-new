...
async function loadFarmPage(cdp, sessionId, baseUrl, sourceVillageId, page = 1, order = 'distance', filtersExpanded = false) {
    ...
    // 체크박스 상태 강제 — 단계별 전략:
    //   1단계 (filtersExpanded=false, 기본): all_village=ON, attacked=OFF, full_losses=OFF
    //     → 공격 중이 아닌 마을만 표시 → 자원 누적된 마을 우선
    //   2단계 (filtersExpanded=true, 0 row 발견 시): attacked=ON 추가
    //     → 공격 중인 마을도 표시 → 거리순 + cooldown으로 추가 공격
    try {
        const changed = await evaluate(cdp, sessionId, `
            (() => {
                ...
                const out = [];
                // 항상: "이 마을 공격만 표시" ON
                const all = document.getElementById('all_village_checkbox');
                if (all && !all.checked) { all.click(); out.push('all-on'); }
                // attacked_checkbox: 1단계 OFF, 2단계 ON
                const att = document.getElementById('attacked_checkbox');
                if (att) {
                    if (expanded && !att.checked) { att.click(); out.push('attacked-on'); }
                    else if (!expanded && att.checked) { att.click(); out.push('attacked-off'); }
                }
                // full_losses_checkbox: 항상 OFF (전멸 보고서 제외)
                const fl = document.getElementById('full_losses_checkbox');
                if (fl && fl.checked) { fl.click(); out.push('fullLosses-off'); }
                return out;
            })()
        `).catch(() => []);
        if (changed && changed.length) {
            log.info(`[동줍] ${sourceVillageId} — 필터 ${filtersExpanded ? '확장' : '초기화'}: ${changed.join(', ')}`);
    ...
    // 페이지 사이즈 200 보장 (Accountmanager.change_pagesize_link 사용 — 캡처 검증)
    // TW 유저 설정에 영구 저장 — 한 번만 POST, 다음부터 자동 200
    try {
        const sizeInfo = await evaluate(cdp, sessionId, `
            (() => {
                const m = document.documentElement.innerHTML.match(/change_pagesize_link\\s*=\\s*['\\\"]([^'\\\"]+)['\\\"]/);
                const link = m ? m[1] : null;
                const csrfM = link ? link.match(/&h=([a-f0-9]+)/) : null;
                const sizeInput = document.querySelector('input[name="page_size"]');
                const currentSize = sizeInput ? parseInt(sizeInput.value) : null;
                return { link, csrf: csrfM ? csrfM[1] : null, currentSize };
            })()
        `).catch(() => null);
        if (sizeInfo?.link && sizeInfo?.csrf && sizeInfo.currentSize !== null && sizeInfo.currentSize < 200) {
            log.info(`[동줍] ${sourceVillageId} — 페이지 사이즈 ${sizeInfo.currentSize} → 200`);
            await evaluate(cdp, sessionId, `
                (async () => {
                    try {
                        const params = new URLSearchParams();
                        params.append('page_size', '200');
                        params.append('h', '${sizeInfo.csrf}');
                        await fetch('${sizeInfo.link}', {
                            method: 'POST',
        ...
        (() => {
            try {
                ...
                for (const tr of trs) {
                    ...
                    let durSec = null;
                    let reqA = null, reqB = null, reqC = null;
                    ...
                    if (btnC) {
                        ...
                        // C 버튼도 hover 시 필요 병력 title에 표시됨 (A/B와 동일 포맷)
                        reqC = parseRequiredUnits(tt);
                        if (!durSec) {
                            const dm = tt.match(/Duration:\\s*(\\d+):(\\d{1,2}):(\\d{1,2})/);
                            if (dm) durSec = parseInt(dm[1])*3600 + parseInt(dm[2])*60 + parseInt(dm[3]);
                        }
                    ...
                    // max_loot 아이콘 레벨 (0~4) — 마지막 공격에서 가져온 자원 가득 정도
                    const mlImg = tr.querySelector('img[src*="max_loot/"]');
                    let maxLootLevel = null;
                    if (mlImg) {
                        const mm = (mlImg.getAttribute('src') || '').match(/max_loot\\/(\\d+)\\.webp/);
                        if (mm) maxLootLevel = parseInt(mm[1]);
                    }

                    // 추정 자원 (wood + stone + iron) — 정찰 보고서 기반
                    // 캡처 검증: <td colspan="3">의 <span class="res">N</span> × 3개
                    // 정찰 없으면 ? 표시되며 res span 없음 → null
                    let estLoot = null;
                    const resSpans = tr.querySelectorAll('span.res');
                    if (resSpans.length >= 3) {
                        let total = 0;
                        for (const sp of resSpans) {
                            const n = parseInt((sp.textContent || '').trim());
                            if (!isNaN(n)) total += n;
                        }
                        estLoot = total;
                    }

                    // 좌표 추출 (적응형 쿨다운에서 사용 — 보고서 평균 loot 매핑)
                    const coordEl = tr.querySelector('a[href*="screen=report"]');
                    const coordTxt = coordEl ? coordEl.textContent : '';
                    const coordM = coordTxt.match(/\\((\\d+)\\|(\\d+)\\)/);
                    const targetX = coordM ? parseInt(coordM[1]) : null;
                    const targetY = coordM ? parseInt(coordM[2]) : null;
                    ...
                    rows.push({
                        ...
                        reportId,
                        durationSec: durSec,
                        requiredA: reqA, requiredB: reqB, requiredC: reqC,
                        maxLootLevel,
                        estLoot,           // wood+stone+iron, null이면 정찰 안 함
                    });
                }
...
async function farmFromVillage(cdp, sessionId, baseUrl, sourceVillage, options = {}) {
    ...
    // 봇 탐지 회피 — 더 느린 클릭 간격 (사람 평균 3-6초/click)
    const delayMin = options.delayMin || 1500;
    const delayMax = options.delayMax || 5500;
    const longPauseChance = options.longPauseChance ?? 0.2;
    ...
    let lastAvailable = {};  // 마지막 page 끝난 후 남은 병력 (stage 2 결정용)

    for (let page = 1; page <= maxPages; page++) {
        let data;
        try {
            data = await loadFarmPage(cdp, sessionId, baseUrl, sourceVillage.id, page, 'distance', !!options._filtersExpanded);
        } catch (e) {
        ...
        // 모든 모드 최소 병력 임계 — 사용자 지정
        // 1) 총 합산 ≥ 6 (1마리만 보내는 경우 절대 없어야 함)
        // 2) 기마(light) 사용되는 경우 light ≥ 6
        const MIN_TROOPS = options.cMinTroops != null ? options.cMinTroops : 6;
        const MIN_LIGHT = options.cMinLight != null ? options.cMinLight : 6;
        const sumReq = (req) => {
            if (!req) return 0;
            let s = 0; for (const n of Object.values(req)) s += (n || 0);
            return s;
        };
        const tooSmall = (req) => {
            if (!req) return false;
            const total = sumReq(req);
            if (total > 0 && total < MIN_TROOPS) return true;
            if ((req.light || 0) > 0 && (req.light || 0) < MIN_LIGHT) return true;
            return false;
        };

        // 각 row별로 어떤 방식으로 공격할지 결정
        // 최소 병력 미달이면 어떤 방식으로도 공격 안 보냄 (6마리 이하 / 기마 6 미만)
        const actions = [];
        let minSkipped = 0;
        for (const row of data.rows) {
            if (mode === 'C' || mode === 'C_OR_A' || mode === 'C_OR_B') {
                if (row.hasC && row.reportId) {
                    if (tooSmall(row.requiredC)) { minSkipped++; continue; }
                    actions.push({ row, method: 'C' });
                } else if (mode === 'C_OR_A' && row.hasA) {
                    if (tooSmall(row.requiredA)) { minSkipped++; continue; }
                    actions.push({ row, method: 'A' });
                } else if (mode === 'C_OR_B' && row.hasB) {
                    if (tooSmall(row.requiredB)) { minSkipped++; continue; }
                    actions.push({ row, method: 'B' });
                }
            } else if (mode === 'A') {
                if (row.hasA) {
                    if (tooSmall(row.requiredA)) { minSkipped++; continue; }
        ...
        if (minSkipped > 0) log.info(`[동줍] ${sourceVillage.name} p${page} — 최소 병력 미달로 ${minSkipped}개 스킵 (총<${MIN_TROOPS} 또는 기마<${MIN_LIGHT})`);

        // 정렬 — 추정 자원 많은 타겟 먼저
        // 캡처 검증: row의 <span class="res">N</span> × 3 (wood/stone/iron) 합 = estLoot
        // 정찰 안 한 마을은 estLoot=null → maxLootLevel × 100 으로 추정 → 그것도 null이면 -1
        const lootScore = (row) => {
            if (row.estLoot != null) return row.estLoot;
            if (row.maxLootLevel != null) return row.maxLootLevel * 100;
            return -1;
        };
        actions.sort((a, b) => lootScore(b.row) - lootScore(a.row));

        if (actions.length === 0) {
            log.info(`[동줍] ${sourceVillage.name} p${page} — 공격 가능 row 없음 (mode=${mode})`);
            // 1단계 끝났는데 0 row → 2단계로 (attacked_checkbox ON) → 거리순 + cooldown
            if (page === 1 && !options._filtersExpanded) {
                log.info(`[동줍] ${sourceVillage.name} — 2단계 진입: attacked_checkbox ON (거리순 + cooldown)`);
        ...
        for (const { row, method } of actions) {
            // 사전 병력 체크 — A/B/C 모두 hover title에서 필요 병력 파싱됨
            // 보유 병력 < 필요 병력이면 헛 클릭 방지 위해 스킵
            const required = method === 'A' ? row.requiredA : (method === 'B' ? row.requiredB : row.requiredC);
            ...
            if (result?.ok) {
                totalSent++;
                decrement(required);
                // 쿨다운 — 같은 타겟에 30분 간격 (사용자 지정)
                // 1단계 (attacked OFF) 필터: TW가 공격중 마을 자동 숨김 — 중복 자동 방지
                // 2단계 (attacked ON): 쿨다운만으로 spam 방지 → 30분마다 반복 공격 가능
                if (recentTargets) {
                    const sentAtMs = Date.now();
                    const cooldownMs = options.cooldownMs || 30 * 60 * 1000;
                    recentTargets.set(row.targetId, {
                        sentAtMs,
                        returnsAtMs: sentAtMs + cooldownMs,
                    });
                }
        ...
        lastAvailable = available;  // 다음 iteration 또는 stage 2 결정용 보존
        if (!data.hasNext) break;
        await sleep(randInt(1000, 2000));
    }

    // 2단계 진입 조건 — 1단계 완료 후 병력 6마리+ 남았으면 (공격중 마을에 추가 발사)
    // 병력 다 썼으면 트래픽 낭비 안 함
    if (!options._filtersExpanded) {
        const FARM_UNITS = ['spear','sword','axe','archer','light','marcher','heavy'];
        let hasMeaningfulTroops = false;
        for (const u of FARM_UNITS) {
            if ((lastAvailable[u] || 0) >= 6) { hasMeaningfulTroops = true; break; }
        }
        if (hasMeaningfulTroops) {
            log.info(`[동줍] ${sourceVillage.name} — 1단계 완료 (${totalSent}회), 병력 남음 → 2단계 진입`);
            await sleep(randInt(1000, 2500));
            try {
                const stage2 = await farmFromVillage(cdp, sessionId, baseUrl, sourceVillage, { ...options, _filtersExpanded: true });
                return {
                    totalSent: totalSent + (stage2.totalSent || 0),
                    totalFailed: totalFailed + (stage2.totalFailed || 0),
                    lastError: stage2.lastError || lastError,
                    earliestReturnMs: !earliestReturnMs ? stage2.earliestReturnMs
                        : (stage2.earliestReturnMs && stage2.earliestReturnMs < earliestReturnMs)
                            ? stage2.earliestReturnMs : earliestReturnMs,
                };
            } catch (e) {
                if (e instanceof BotProtectionError) throw e;
                log.warn(`[동줍] ${sourceVillage.name} 2단계 실패: ${e.message}`);
            }
        } else {
            log.info(`[동줍] ${sourceVillage.name} — 1단계 완료 (${totalSent}회), 병력 모두 소진 → 2단계 스킵`);
        }
    }
