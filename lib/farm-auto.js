    ...
    for (let page = 1; page <= maxPages; page++) {
        ...
        for (const { row, method } of actions) {
            ...
            if (result?.ok) {
                totalSent++;
                decrement(required);
                // 쿨다운 — 최소 40분 (자원 생산 시간 확보) + 왕복 시간 중 큰 값
                // 26점 바바리안 기준 시간당 30/자원 생산 → 40분 ≈ 20/자원 누적
                // 가까운 마을: 왕복 < 40분 → 40분 cooldown (생산 누적 대기)
                // 먼 마을: 왕복 > 40분 → 왕복 cooldown (자연 도착 대기)
                if (recentTargets) {
                    const sentAtMs = Date.now();
                    const roundtripMs = (row.durationSec || 600) * 2 * 1000;
                    const minCooldownMs = 40 * 60 * 1000; // 40분
                    const buffer = 30000;
                    const cooldownMs = Math.max(minCooldownMs, roundtripMs) + buffer;
