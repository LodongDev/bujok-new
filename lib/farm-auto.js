...
async function farmFromVillage(cdp, sessionId, baseUrl, sourceVillage, options = {}) {
    ...
    for (let page = 1; page <= maxPages; page++) {
        ...
        // 최소 병력 임계는 C에만 적용 (A/B는 사용자가 직접 설정한 템플릿이라 그대로 보냄)
        const actions = [];
        let minSkipped = 0;
        ...
        if (minSkipped > 0) log.info(`[동줍] ${sourceVillage.name} p${page} — C 최소 병력 미달로 ${minSkipped}개 스킵 (총<${MIN_TROOPS} 또는 기마<${MIN_LIGHT})`);
        ...
        for (const { row, method } of actions) {
            ...
            if (result?.ok) {
                ...
                if (recentTargets) {
                    ...
                    const cooldownMs = options.cooldownMs || 7.5 * 60 * 1000;  // 7.5분 (30/4)
