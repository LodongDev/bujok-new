        ...
        (() => {
            try {
                ...
                for (const tr of trs) {
                    ...
                    // 캡처 검증: <img src=".../max_loot/N.webp"> N=0(빈) ~ 4(가득)
                    // 정찰 안 했어도 모든 row에 있음 — 유일한 row별 loot 신호
...
async function farmFromVillage(cdp, sessionId, baseUrl, sourceVillage, options = {}) {
    ...
    for (let page = 1; page <= maxPages; page++) {
        ...
        // 정렬 — 수집 가능 자원 많은 타겟 먼저
        // 사용 데이터 (캡처 검증된 실측 데이터):
        //   1. targetStatsByCoords[x|y].avgLoot — 보고서수집기가 누적한 실제 평균 loot
        //   2. row.maxLootLevel (0~4) — 마지막 공격에서 가져온 자원의 가득 정도 (TW 표시 아이콘)
        // 점수: avgLoot 있으면 그 값 그대로, 없으면 maxLootLevel × 100 (대략 추정)
        const stats = options.targetStatsByCoords;
        const lootScore = (row) => {
            if (stats && row.targetX != null && row.targetY != null) {
                const s = stats.get(`${row.targetX}|${row.targetY}`);
                if (s?.avgLoot != null) return s.avgLoot;
            }
            // 보고서 없는 타겟 — 아이콘 레벨로 추정 (0=가장 적음, 4=가득)
