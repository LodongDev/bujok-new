        ...
        (() => {
            try {
                ...
                for (const tr of trs) {
                    ...
                    let expLootA = null, expLootB = null, expLootC = null;

                    // 버튼 title의 "header ressources ... </span>NNN" — 예상 자원량 (캡처 검증)
                    const parseExpectedLoot = (tt) => {
                        if (!tt) return null;
                        const re1 = /header\\s+ressources[^>]*>\\s*<\\/span>\\s*(\\d+)/;
                        const m1 = tt.match(re1);
                        if (m1) return parseInt(m1[1]);
                        const re2 = /header\\s+ressources&quot;[^&]*&lt;\\/span&gt;\\s*(\\d+)/;
                        const m2 = tt.match(re2);
                    ...
                    rows.push({
                        ...
                        expectedLootA: expLootA, expectedLootB: expLootB, expectedLootC: expLootC,
                    });
                }
...
async function farmFromVillage(cdp, sessionId, baseUrl, sourceVillage, options = {}) {
    ...
    for (let page = 1; page <= maxPages; page++) {
        ...
        for (const row of data.rows) {
            if (mode === 'C') {
                if (row.hasC && row.reportId) actions.push({ row, method: 'C', expLoot: row.expectedLootC });
            } else if (mode === 'A') {
                if (row.hasA) actions.push({ row, method: 'A', expLoot: row.expectedLootA });
            } else if (mode === 'B') {
                if (row.hasB) actions.push({ row, method: 'B', expLoot: row.expectedLootB });
            } else if (mode === 'C_OR_A') {
                if (row.hasC && row.reportId) actions.push({ row, method: 'C', expLoot: row.expectedLootC });
                else if (row.hasA) actions.push({ row, method: 'A', expLoot: row.expectedLootA });
            } else if (mode === 'C_OR_B') {
                if (row.hasC && row.reportId) actions.push({ row, method: 'C', expLoot: row.expectedLootC });
                else if (row.hasB) actions.push({ row, method: 'B', expLoot: row.expectedLootB });
            }
        }

        // 예상 자원량(expectedLoot) 내림차순 정렬 — 많이 가져올 수 있는 타겟 먼저 발사
        // 캡처 검증: A/B/C 버튼 title의 "header ressources ... </span>NNN" 값
        // null인 항목은 마지막 (정보 없음 = 후순위)
        actions.sort((a, b) => {
            const la = a.expLoot != null ? a.expLoot : -1;
            const lb = b.expLoot != null ? b.expLoot : -1;
            return lb - la;
        });
