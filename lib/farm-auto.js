...
async function loadFarmPage(cdp, sessionId, baseUrl, sourceVillageId, page = 1, order = 'date') {
...
async function farmFromVillage(cdp, sessionId, baseUrl, sourceVillage, options = {}) {
    ...
    for (let page = 1; page <= maxPages; page++) {
        ...
        // 라운드 간 같은 타겟 반복 공격 방지 — recentTargets에 있는 타겟은 cooldown 중이면 스킵
        ...
        if (recentTargets) {
            ...
            for (const a of actions) {
                const last = recentTargets.get(a.row.targetId);
            ...
            if (cooldownSkipped > 0) {
                log.info(`[동줍] ${sourceVillage.name} 쿨다운으로 ${cooldownSkipped}개 타겟 스킵 (10분 이내 재공격 방지)`);
            }
        ...
        for (const { row, method } of actions) {
            ...
            if (result?.ok) {
                totalSent++;
                decrement(required);
                // 쿨다운 추적 — 이 타겟에 다시 보내지 않도록 기록
                if (recentTargets) recentTargets.set(row.targetId, Date.now());
