    ...
    for (let page = 1; page <= maxPages; page++) {
        ...
        for (const { row, method } of actions) {
            ...
            if (result?.ok) {
                ...
                decrement(required);
                // 적응형 쿨다운 추적
                // 기본: 부대 복귀까지 (왕복 + 30초 버퍼)
                // + 평균 loot 기반 추가 대기 (자원 생산 시간 고려):
                //   - 평균 loot 매우 낮음 (<20): 빈 마을 → 추가 30분 대기 (자원 누적 시간)
                //   - 낮음 (20~50): 추가 10분
                //   - 보통 (50~150): 추가 0 (즉시 재공격 가능)
                //   - 높음 (>150): 추가 0 (자주 가야 좋음)
                if (recentTargets) {
                    const sentAtMs = Date.now();
                    const roundtripMs = (row.durationSec || 600) * 2 * 1000;
                    const buffer = 30000;
                    // 좌표 기반 통계 조회
                    const coordKey = row.targetX != null && row.targetY != null
                        ? `${row.targetX}|${row.targetY}`
                        : null;
                    const stats = (options.targetStatsByCoords && coordKey)
                        ? options.targetStatsByCoords.get(coordKey)
                        : null;
                    let extraWaitMs = 0;
                    if (stats && stats.attackCount >= 3) { // 최소 3회 이상 데이터 있어야 신뢰
                        if (stats.avgLoot < 20) extraWaitMs = 30 * 60 * 1000;       // 30분
                        else if (stats.avgLoot < 50) extraWaitMs = 10 * 60 * 1000;  // 10분
                        // 50 이상은 추가 대기 없음 (즉시 cooldown만 적용)
                    }
                    recentTargets.set(row.targetId, {
                        sentAtMs,
                        returnsAtMs: sentAtMs + roundtripMs + buffer + extraWaitMs,
