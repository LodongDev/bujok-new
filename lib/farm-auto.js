...
async function farmFromVillage(cdp, sessionId, baseUrl, sourceVillage, options = {}) {
    ...
    for (let page = 1; page <= maxPages; page++) {
        ...
        // C 모드 최소 병력 임계 — 장벽 있는 마을은 10+ 병력 필요 (사용자 지정)
        // requiredC 합산 < 10이면 스킵 (낭비 방지)
        const C_MIN_TROOPS = options.cMinTroops != null ? options.cMinTroops : 10;
        const sumReq = (req) => {
            if (!req) return 0;
        ...
        for (const row of data.rows) {
            if (mode === 'C') {
                if (row.hasC && row.reportId) {
                    if (sumReq(row.requiredC) > 0 && sumReq(row.requiredC) < C_MIN_TROOPS) { cMinSkipped++; continue; }
                ...
                if (row.hasC && row.reportId) {
                    if (sumReq(row.requiredC) > 0 && sumReq(row.requiredC) < C_MIN_TROOPS) {
                        // C 너무 적음 → A 폴백 시도
                        if (row.hasA) actions.push({ row, method: 'A' });
                        else cMinSkipped++;
                        continue;
                ...
                else if (row.hasB) actions.push({ row, method: 'B' });
            }
        }
        if (cMinSkipped > 0) log.info(`[동줍] ${sourceVillage.name} p${page} — C 최소 병력 미달로 ${cMinSkipped}개 스킵`);
