...
async function detectVillages(cdp, sessionId, baseUrl) {
    ...
    // 페이지 크기 확인 — 기본 25라서 25개 넘으면 잘려 보임
    // 캡처 검증: pagination_form의 page_size input에 현재 값, change_page_size POST로 변경
    const pageInfo = await evaluate(cdp, sessionId, `
        (() => {
            const input = document.querySelector('input[name="page_size"]');
            const currentSize = input ? parseInt(input.value) : 25;
            const csrfMatch = document.documentElement.innerHTML.match(/&h=([a-f0-9]+)/);
            const csrf = csrfMatch ? csrfMatch[1] : null;
            const villageCount = document.querySelectorAll('span.quickedit-vn[data-id]').length;
            return { currentSize, csrf, villageCount };
        })()
    `);

    // 페이지 크기보다 마을 많을 가능성 → 1000으로 늘림
    if (pageInfo?.csrf && pageInfo.currentSize < 1000) {
        log.info(`마을 페이지 크기 ${pageInfo.currentSize} → 1000으로 변경 (현재 표시 ${pageInfo.villageCount}개)`);
        try {
            await evaluate(cdp, sessionId, `
                (async () => {
                    const params = new URLSearchParams();
                    params.append('page_size', '1000');
                    params.append('h', '${pageInfo.csrf}');
                    const res = await fetch('${baseUrl}/game.php?screen=overview_villages&mode=combined&action=change_page_size', {
                        method: 'POST',
            ...
            `);
            // 페이지 다시 로드 (모든 마을 표시되게)
            await navigate(cdp, sessionId, url);
            await waitForLoad(cdp, sessionId);
            await sleep(randInt(500, 1000));
        } catch (e) { log.warn('페이지 크기 변경 실패: ' + e.message); }
    }
