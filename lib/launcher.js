...
function launchChrome(cdpPort = 9222) {
    ...
    const child = spawn(chromePath, [
        ...
        '--start-maximized',
        // Site isolation 비활성화 — hCaptcha OOPIF를 같은 프로세스로 강제
        // CDP Target.attachedToTarget / Runtime.executionContextCreated 이벤트가 발생하게 함
        // (실측: 캡처 검증된 진단 로그에서 OOPIF 격리로 인해 이벤트 미발생 확인)
...
async function detectServersFromGame(cdp, sessionId) {
    ...
    // api/world_switch로 전체 서버 목록 — 디버그 정보 포함
    const detect = await evaluate(cdp, sessionId, `
        (async () => {
            try {
                // village 파라미터 포함 (캡처 검증된 URL 형식)
                const villageMatch = document.documentElement.innerHTML.match(/village=(\\d+)/);
                const vparam = villageMatch ? '&village=' + villageMatch[1] : '';
                const url = location.origin + '/game.php?screen=api&ajax=world_switch' + vparam;
                ...
                // 응답 형식 두 가지 — {html:...} 또는 {response:{html:...}}
                const html = json?.response?.html || json?.html || '';
                const matches = [...html.matchAll(/submit_login\\('server_([a-z0-9]+)'\\)/gi)];
                const servers = matches.map(m => m[1]);
                return {
                    url, status,
                    bodyLen: text.length,
    ...
    `);
    log.info(`[서버감지] ${detect.url} status=${detect.status} bodyLen=${detect.bodyLen} htmlLen=${detect.htmlLen} servers=${JSON.stringify(detect.servers)}`);
    if (detect.error) log.warn(`[서버감지] error: ${detect.error}`);
    if (detect.servers && detect.servers.length === 0 && detect.bodyStart) {
        log.warn(`[서버감지] 응답 시작: ${detect.bodyStart}`);
    }
    const servers = detect.servers || [];
...
module.exports = { findChrome, killAllChrome, launchChrome, waitForCDP, loginAndDetectServers, enterServer, detectServersFromGame };
