...
class ScavengeQueue {
    constructor(cdp, sessionId, baseUrl, scheduler, botLock, onBotProtection) {
        ...
        this.unitsByVillage = {};  // { villageId: ['spear','axe',...] } 마을별 허용 병종
        this.timer = null;
        this.running = false;
        this.stopped = false;
    }
    ...
    // 큐 시작 — 전체 마을 초기화
    // options.unitsByVillage: { villageId: ['spear','axe',...] }
    async start(villages, options = {}) {
        this.stopped = false;
        this.villageIds = new Set(villages.map(v => v.id));
        this.unitsByVillage = options.unitsByVillage || {};
        this.queue = [];
    ...
    // 마을별 병종 화이트리스트 동적 업데이트
    updateUnitsByVillage(unitsByVillage) {
        this.unitsByVillage = unitsByVillage || {};
        log.info(`[큐] 마을별 병종 갱신 — ${Object.keys(this.unitsByVillage).length}개 마을 설정`);
    }
    ...
    async tick() {
        ...
        for (const item of ready) {
            ...
            try {
                ...
                const allowedUnits = this.unitsByVillage[item.villageId];
                const result = await scavengeVillage(this.cdp, this.sessionId, this.baseUrl, item.villageId, { allowedUnits });
