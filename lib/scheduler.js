...
class Scheduler {
    ...
    async prepareAndFire(atk) {
        ...
        // TW 서버 처리 시간 마진 — POST 받은 후 명령 등록까지 ~1초 (실측 1초 늦게 도착 관측)
        // 1초 일찍 발사 = 정확히 목표 시각에 도착
        const TW_PROCESSING_MARGIN_MS = 1000;
        atk.fireTime = atk.arrivalTime - atk.travelTimeMs - freshDrift - halfLat - TW_PROCESSING_MARGIN_MS;
        atk.firingDriftCompMs = -freshDrift;
        atk.firingHalfLatMs = -halfLat;
        atk.firingProcessingMarginMs = -TW_PROCESSING_MARGIN_MS;
        const waitMs = atk.fireTime - Date.now();
        if (waitMs < -5000) throw new Error(`발사 시점 이미 ${Math.round(-waitMs/1000)}초 지남`);
        log.info(`[예약 #${atk.id}] ch 받음. 발사까지 ${Math.round(Math.max(0, waitMs)/1000)}초 (Duration ${Math.round(confirm.durationMs/1000)}초, drift comp ${-Math.round(freshDrift)}ms, halfLat ${-halfLat}ms)`);
