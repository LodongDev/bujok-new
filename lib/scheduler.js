...
class Scheduler {
    constructor(cdp, sessionId, baseUrl) {
        ...
        this.driftMs = 0;       // PC 시계 vs 서버 시계 (양수=서버가 PC보다 앞, 음수=PC가 앞)
    }
    ...
    async prepareAndFire(atk) {
        ...
        // 같은 confirm 응답의 time_generated로 fresh drift 계산 (추가 요청 없음)
        let freshDrift = this.driftMs;
        if (confirm.serverMs) {
            const browserMid = (confirm.t0 + confirm.t1) / 2;
            freshDrift = confirm.serverMs - browserMid;
            this.driftMs = freshDrift;  // 캐시 갱신
            log.info(`[예약 #${atk.id}] fresh drift: ${Math.round(freshDrift)}ms (confirm 응답에서 추출)`);
        }
        // browser fire = arrivalTime - travelMs - drift - halfLat
        //   - drift: 서버가 PC보다 앞이면 양수. 보정: -drift 추가
        //   - halfLat: POST 한 방향 latency (round trip/2)
        const halfLat = confirm.latencyMs ? Math.round(confirm.latencyMs / 2) : 0;
        // 초기 계산 — T-5s preflight calibration 전 임시값 (목표: arrival 정확히)
        atk.fireTime = atk.arrivalTime - atk.travelTimeMs - freshDrift - halfLat;
        atk.firingDriftCompMs = -freshDrift;
        atk.firingHalfLatMs = -halfLat;
        const waitMs = atk.fireTime - Date.now();
        if (waitMs < -5000) throw new Error(`발사 시점 이미 ${Math.round(-waitMs/1000)}초 지남`);
        log.info(`[예약 #${atk.id}] ch 받음. 임시 fire ${new Date(atk.fireTime).toISOString().slice(11,23)} (Duration ${Math.round(confirm.durationMs/1000)}초, drift ${Math.round(freshDrift)}ms, halfLat ${halfLat}ms)`);

        // 3a. T-5초 preflight calibration — 실측 drift + latency 다시 측정해서 fireTime 보정
        //     confirm POST 후 3분 지나면 network/drift 변화 가능 → 발사 직전 fresh 측정
        const PREFLIGHT_LEAD_MS = 5000;
        const preflightFireAt = atk.fireTime - PREFLIGHT_LEAD_MS;
        const waitToPreflight = preflightFireAt - Date.now();
        if (waitToPreflight > 200) await sleep(waitToPreflight - 100);
        if (atk.status === 'cancelled') return;

        try {
            const cal = await this.preflightCalibration(atk);
            if (cal?.serverMs) {
                const browserMid = (cal.t0 + cal.t1) / 2;
                const calDrift = cal.serverMs - browserMid;
                const calHalfLat = Math.round((cal.t1 - cal.t0) / 2);
                // 보정된 fireTime — 실측 drift + halfLat 적용
                const oldFireTime = atk.fireTime;
                atk.fireTime = atk.arrivalTime - atk.travelTimeMs - calDrift - calHalfLat;
                const delta = atk.fireTime - oldFireTime;
                atk.firingDriftCompMs = -calDrift;
                atk.firingHalfLatMs = -calHalfLat;
                log.info(`[예약 #${atk.id}] preflight: drift=${Math.round(calDrift)}ms, halfLat=${calHalfLat}ms (round trip ${cal.t1 - cal.t0}ms), fireTime 조정 ${delta >= 0 ? '+' : ''}${delta}ms`);
            } else {
                log.warn(`[예약 #${atk.id}] preflight 실패 — 초기 fireTime 사용`);
            }
        } catch (e) {
            log.warn(`[예약 #${atk.id}] preflight 에러: ${e.message}`);
        }

        // 3b. 정밀 대기 (sleep로 100ms 전까지, 그 후 spinning)
    ...
    // T-5s preflight — 발사 직전 실측 drift + latency
    // scavenge_api 응답의 time_generated_ms 사용 (가벼운 fetch)
    async preflightCalibration(atk) {
        const villageId = atk.sourceVillageId;
        return await evaluate(this.cdp, this.sessionId, `
            (async () => {
                try {
                    const t0 = Date.now();
                    const r = await fetch('${this.baseUrl}/game.php?village=${villageId}&screen=scavenge_api&ajax=villages&village_ids%5B%5D=${villageId}', {
        ...
        return await evaluate(this.cdp, this.sessionId, `
            (async () => {
                try {
                    ...
                    // 같은 응답에서 time_generated 추출 (server UTC ms) → 발사 직전 fresh drift 계산
                    const tgM = text.match(/[\"']time_generated[\"']\\s*[:]\\s*(\\d+)/);
                    const serverMs = tgM ? parseInt(tgM[1]) : null;
                    return {
                        ch: chM ? chM[1] : null,
                        durationMs,
                        latencyMs: t1 - t0,
    ...
    async postCommand(atk, formData, ch) {
        ...
        return await evaluate(this.cdp, this.sessionId, `
            (async () => {
                try {
                    const params = new URLSearchParams();
                    // 공격: attack=true + submit_confirm=Send attack
                    // 원군: support=true + submit_confirm=Send support
                    params.append(${isAttack ? "'attack'" : "'support'"}, 'true');
                    params.append('ch', '${ch}');
                    ...
                    params.append('submit_confirm', ${isAttack ? "'Send attack'" : "'Send support'"});
                    params.append('h', '${formData.csrf}');
                    const r = await fetch('${url}', {
                        method: 'POST',
