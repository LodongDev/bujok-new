...
class Scheduler {
    ...
    async prepareAndFire(atk) {
        ...
        // arrivalTime은 server clock UTC 기준. browser fire 시점 = arrivalTime - travelMs - drift - halfLat
        //   - drift: PC 시계와 서버 시계 차이 (서버가 PC보다 앞이면 양수). PC가 fire 일찍 → 서버 더 일찍 받음.
        //     보정: PC fire를 +drift 늦춰서 서버 받는 시점을 맞춤.
        //   - halfLat: POST 가 서버 도달까지 걸리는 시간 (round trip / 2). 미리 fire해야 시간 맞음.
        const halfLat = confirm.latencyMs ? Math.round(confirm.latencyMs / 2) : 0;
        atk.fireTime = atk.arrivalTime - atk.travelTimeMs - this.driftMs - halfLat;
        atk.firingDriftCompMs = -this.driftMs;
        ...
        log.info(`[예약 #${atk.id}] ch 받음. 발사까지 ${Math.round(Math.max(0, waitMs)/1000)}초 (Duration ${Math.round(confirm.durationMs/1000)}초, drift comp ${-this.driftMs}ms, halfLat ${-halfLat}ms)`);
