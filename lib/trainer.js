...
// 병종별 인구 점유 (TW 표준 — gameData.unit_pop에서 검증 가능)
const UNIT_POP = {
        ...
        (() => {
            try {
                ...
                return {
                    ...
                    pop: parseInt(v.pop) || 0,
                    popMax: parseInt(v.pop_max) || 0,
                    queue,
...
async function trainVillage(cdp, sessionId, baseUrl, village, trainPlan) {
    ...
    for (const item of trainPlan) {
        ...
        try {
            ...
            // 인구 사전 체크 — 발주할 병종의 인구 점유 vs 남은 인구
            // pop_max - pop = 남은 인구. unit_pop * count 만큼 필요.
            const unitPop = UNIT_POP[item.unit] || 0;
            const needPop = unitPop * (item.count || 1);
            const freePop = (data.popMax || 0) - (data.pop || 0);
            if (unitPop > 0 && needPop > freePop) {
                log.info(`[양성] ${village.name} ${item.building}/${item.unit}: 인구 부족 (필요 ${needPop}, 남은 ${freePop}) — 스킵`);
                continue;
            }
